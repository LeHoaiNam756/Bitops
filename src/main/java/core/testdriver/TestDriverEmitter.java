...
public class TestDriverEmitter {
    ...
    public static String emitDriver(String packageName, String className, String methodName,
        ...
        sb.append("import com.fasterxml.jackson.databind.JsonNode;\n")
                ...
                .append("import core.instrument.TraceKind;\n")
    ...
    private static void emitMain(StringBuilder sb) {
        sb.append("    public static void main(String[] args) {\n")
                ...
                .append("                resultJson.set(\"branchTrace\", branchTraceJson(mapper));\n")
    ...
    private static void emitHelpers(StringBuilder sb) {
        sb.append("    private static Object convert(JsonNode value, String typeName) {\n")
                ...
                .append("        java.util.Set<Integer> nodeIds = TraceRecorder.coveredStatementNodeIdsSnapshot();\n")
                .append("        if (nodeIds.isEmpty()) nodeIds = TraceRecorder.coveredBranchOutcomeIdsSnapshot();\n")
                .append("        if (nodeIds.isEmpty()) nodeIds = TraceRecorder.coveredNodeIdsSnapshot();\n")
                .append("        for (Integer nodeId : nodeIds) {\n")
                .append("            array.add(nodeId);\n")
                .append("        }\n")
                .append("        return array;\n")
                .append("    }\n\n")
                .append("    private static ArrayNode branchTraceJson(ObjectMapper mapper) {\n")
                .append("        ArrayNode array = mapper.createArrayNode();\n")
                .append("        for (TraceRecorder.OrderedTraceEvent event : TraceRecorder.orderedEventsSnapshot()) {\n")
                .append("            if (event.kind() == TraceKind.COND_T || event.kind() == TraceKind.COND_F) {\n")
                .append("                ObjectNode item = mapper.createObjectNode();\n")
                .append("                item.put(\"nodeId\", event.kind() == TraceKind.COND_T ? event.nodeId() / 2 : (event.nodeId() - 1) / 2);\n")
                .append("                item.put(\"edgeKind\", event.kind() == TraceKind.COND_T ? \"TRUE\" : \"FALSE\");\n")
                .append("                array.add(item);\n")
                .append("            }\n")
                .append("        }\n")
                .append("        return array;\n")
                .append("    }\n");
