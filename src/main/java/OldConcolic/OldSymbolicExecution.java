...
public class OldSymbolicExecution {
    ...
    public OldSymbolicExecution(List<ASTNode> parameterList, List<FindPath.PathNode> testPath) {
    ...
    public void execute() {
        ...
        for (FindPath.PathNode pathNode : testPath) {
            ...
            AstNode rewriteAstNode = OldAstNode.executeASTNode(astNode, memoryModel);
            if (currentNode instanceof CfgBoolExprNode && isRelatedToParameter) {
                if (!pathNode.decision) {
                    ...
                    rewriteAstNode = OldPrefixExpressionNode.executePrefixExpressionNode(prefixExpressionNode,
                            memoryModel);
                }
                ...
                Expr<?> z3Expr = OldExpressionNode.convertAstNodeToZ3ExprOld(rewriteAstNode, ctx, memoryModel);
                BoolExpr constraint;
    ...
    private void executeParameters(Context ctx) {
        for (ASTNode astNode : parameterList) {
            OldAstNode.executeASTNode(astNode, memoryModel);
    ...
    private static Object convertEvaluatedResultToJavaType(Expr evaluatedResult, Class<?> parameterClass) {
        ...
        if (evaluatedResult instanceof IntNum) {
            IntNum intNum = (IntNum) evaluatedResult;
            ...
            if ("int".equals(className)) {
                ...
                throw new RuntimeException("Unsupported integer type: " + className);
            }
        } else if (evaluatedResult instanceof RatNum) {
            RatNum ratNum = (RatNum) evaluatedResult;
            IntNum numerator = ratNum.getNumerator();
            IntNum denominator = ratNum.getDenominator();
            double val = (numerator.getInt64() * 1.0) / denominator.getInt64();

            if ("float".equals(className)) {
                if (Double.isNaN(val)) {
                    return Float.NaN;
                } else if (Double.isInfinite(val)) {
                    return val < 0 ? Float.NEGATIVE_INFINITY : Float.POSITIVE_INFINITY;
                } else {
                    return (float) val;
                }
                ...
                if (Double.isNaN(val)) {
                    ...
                    return val < 0 ? Double.NEGATIVE_INFINITY : Double.POSITIVE_INFINITY;
                } else {
                    return val;
                }
            } else {
